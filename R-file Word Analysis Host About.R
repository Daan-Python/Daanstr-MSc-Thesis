...
Listing$host_about <- gsub("sink", "drainage", Listing$host_about, ignore.case = TRUE) #Sink is likely a noun instead of negative adjective
Listing$host_about <- gsub("bunk", "bed", Listing$host_about, ignore.case = TRUE)      #Bunk is likely a noun instead of negative adjective
Listing$host_about <- as.character(Listing$host_about)  %>%
  tolower() %>%
...
Listing$host_about <- gsub("[^0-9A-Za-z///' ]", "", Listing$host_about, ignore.case = TRUE) 
...
pol <- polarity(Listing[,"host_about"])$all
...
all_informative_words%>%
  ...
  ggtitle("Word Frequencies: informative host_about")

# Obtain word frequencies in persuasive texts
all_persuasive_words <- Listing  %>% filter(style == "Persuasive")
all_persuasive_words <- (all_persuasive_words) %>% unnest_tokens(word,host_about) %>% anti_join(stop_words, by = "word")
all_persuasive_words%>%
  ...
  coord_flip() +
  theme(text = element_text(size = 17)) +
  ggtitle("Word Frequencies: persuasive host_abouts ")
